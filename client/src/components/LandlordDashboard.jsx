...
const LandlordDashboard = forwardRef((props, ref) => {
  const { account, fetchAllVaults, invalidateVaultsCache } = useBlockchain();
  ...
  const loadVaults = useCallback(async (forceRefresh = false) => {
    ...
    try {
      setIsLoading(true);
      console.log('Loading vaults for landlord...');
      
      // Use cached fetch
      const allVaults = await fetchAllVaults(forceRefresh);
      
      // Filter for landlord's vaults
      const landlordVaults = allVaults.filter(vault => 
        account.toLowerCase() === vault.landlord.toLowerCase()
      );
      
      console.log(`Found ${landlordVaults.length} vaults for landlord out of ${allVaults.length} total`);
      
      // Sort vaults
      const sortedVaults = landlordVaults.sort((a, b) => {
  ...
  useImperativeHandle(ref, () => ({
    refresh: () => loadVaults(true) // Force refresh on explicit call
  }));
  ...
  if (vaults.length === 0) {
    return (
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-12 text-center">
        <div className="w-16 h-16 rounded-xl bg-gradient-to-r from-blue-600/20 to-purple-600/20 flex items-center justify-center text-3xl mb-4 mx-auto">
          🏠
        </div>
        <h3 className="text-2xl font-bold text-white mb-2">No Agreements Yet</h3>
        <p className="text-gray-400 mb-6">Create your first rental agreement to get started</p>
      </div>
    );
  }
...
LandlordDashboard.displayName = 'LandlordDashboard';
